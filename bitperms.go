...
// PermissionValue is an uint64 with some additional functions attached.
...
// HasFlag takes in an uint64 permission flag and runs a logical AND operation on the PermissionValue and the flag to
...
func (pv PermissionValue) HasFlag(flag uint64) bool {
	res := uint64(pv) & flag
...
func (pv PermissionValue) HasFlags(flags ...uint64) bool {
	flag := uint64(0)
